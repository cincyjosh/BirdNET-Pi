...
find $HOME/Bird* -not -user $USER -execdir chown $USER:$USER {} \+
chmod 666 $HOME/BirdNET-Pi/scripts/*.txt
chmod 666 $HOME/BirdNET-Pi/*.txt
find $HOME/BirdNET-Pi -path "$HOME/BirdNET-Pi/birdnet" -prune -o -type f ! -perm /g=w -exec chmod g+w {} \;
chmod g+r $HOME

# remove world-writable perms
chmod -R o-w $HOME/BirdNET-Pi/templates/*
